...
We could assign a {\em numerical return} to the outcomes with the largest value for the goal we would like the agent to achieve and simply tweak the agent to take {\em actions} (out of the set of possible actions at any given time) so as to maximize the return.  
...
\item Find the best $\pi_\star$ such that $\max_a q_{\pi_\star}(s,a) \ge \max_a q_\pi(s,a)~~ \forall ~\pi,s$
\end{itemize}
...
\subsection{From the Trajectory Return}
...
\subsection{From the Value Function}
