...
Imagine a problem where we have an {\em agent} that interacts with and {\em environment} and {\em we} want it to achieve a goal without explicitly telling it what to do? The goal itself may have the reward spread over the entire journey (think stock returns coming from readjusting a portfolio) or have just one reward in the end (checkmate!).

We could assign a {\em numerical reward} to the actual goal (if it already is a numerical value we can keep it as is or modulate it) and simply ask the agent to take {\em actions} (out of the set of possible actions at any given time) so as to maximize the reward.  We could also make additional rewards along the way to goad the agent into {\em what we believe} is an optimal path to the ultimate reward.
...
\ee
Here, we start in a state $S_0$ (which comprises of the {\em state of the environement} as well as the {\em internal state of the agent} and may be only partially visible to the agent. The agent then takes the action $A_0$. This results in reward which we {\em choose to label $R_1$}. This explains why $R_0$ is cancelled out since the game has just started. This goes on till the game terminates at time $T$. Note it is not necessary that a game terminates.
...
Note we have given in to the Markovian fantasy above in that we assume the next step depends only on the current state. In theory, this is not a big assumption, as one can always {\em define} the state to contain all the past information that is required. In practice though it is a strong assumption that is frequently made without much though largely due to the complexity involved in the alternative assumption.

Now with this Markovian fantasy the thing the agent needs to do at time $t$ is to decide what action to take in the state $S_t$. In order to do this the agent needs to have a {\em policy} from which actions are sampled $A_t \sim \pi(A_t|S_t)$. This may be a deterministic or stochastic policy. Now are come to the two main problems of Reinforcement Learning
\begin{itemize}
\item What are $p(R_{t+1} | S_t,A_t)$ and $p(S_{t+1}|S_t,A_t)$?
\item What should $\pi(A_t|S_t)$ be?
\end{itemize}

The rest of the lecture will be  discussing how to solve these problems.

\section{Markov Decision Processes}

A MDP is a sequence of states, actions and rewards as shown in eqn \ref{MDP}. Note that
\bea
p(S_{t_2}|S_{t_1}) &=& \sum_{S_t,A_t} p(S_{t_2}|S_t,A_t) p(S_t,A_t|S_{t_1}) \\
&=& \sum_{S_t,A_t} p(S_{t_2}|S_t,A_t) \pi(A_t|S_t) p(S_t|S_{t_1}) 
\eea
as long as $t_1 < t < t_2$.


More generally we can write the partition function
\bea
Z_\pi = \sum_{S_{0:T}, A_{0:T-1}} \prod_{t=0}^{T-1} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0)
\eea
\bea
\mathbb E[R_{\tau+1}|\pi] &=&  \sum_{S_{0:T}, A_{0:T-1}}  R_{\tau+1} p(R_{t+1}|S_t,A_t) \prod_{t=0}^{T-1} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0) \nn
&=& \sum_{S_{0:t}, A_{0:t}}  R_{\tau+1} p(R_{\tau+1}|S_\tau,A_\tau) \prod_{t=0}^{\tau-1} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t)  p(S_0)
\eea
The actions taken and states coming  after the reward in question do not impact the expectation value. 
...
We define
\bea
J_\theta &=& \sum_{S_{0:T}, A_{0:T-1}} \sum_{R_{1:T-1}} [\sum_{a=0}^{T-1} \gamma^a R_{a+1}]   \prod_{t=0}^{T-1} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0) \nn
&=& \sum_{S_{0:T}, A_{0:T-1}} \sum_{R_{1:T-1}} [\sum_{a=0}^{T-1} \gamma^a R_{a+1}]   \prod_{t=0}^{a} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0)
\eea
We then have
\bea
\partial_\theta J_\theta &=& \sum_{S_{0:T}, A_{0:T-1}} \sum_{R_{1:T-1}} [\sum_{a=0}^{T-1} \gamma^a R_{a+1}]  [ \sum_{b=0}^{T-1} \partial_\theta \log \pi(A_b|S_b)] \prod_{t=0}^{T-1} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0) \nn
 &=& \sum_{S_{0:T}, A_{0:T-1}} \sum_{R_{1:T-1}} [\sum_{a=0}^{T-1} \gamma^a R_{a+1}]  [ \sum_{b=0}^a \partial_\theta \log \pi(A_b|S_b)] \prod_{t=0}^a p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0) \nn
 &=& \sum_{S_{0:T}, A_{0:T-1}} \sum_{R_{1:T-1}} [\sum_{a=b}^{T-1} \gamma^a R_{a+1}]  [ \sum_{b=0}^{T-1} \partial_\theta \log \pi(A_b|S_b)] \prod_{t=0}^a p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0) \nn
 &=& \sum_{S_{0:T}, A_{0:T-1}} \sum_{R_{1:T-1}} [\gamma^b \sum_{k=0}^{T-1-b} \gamma^k R_{b+k+1}]  [ \sum_{b=0}^{T-1} \partial_\theta \log \pi(A_b|S_b)] \prod_{t=0}^{b+k} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0)  \nn
 \eea
 We can use the relation between returns and rewards to rewrite the above as
 \bea
  \partial_\theta J_\theta &=& \sum_{S_{0:T}, A_{0:T-1}} \sum_{R_{1:T-1}}   [ \sum_{b=0}^{T-1} \gamma^b G_b \partial_\theta \log \pi(A_b|S_b)] \prod_{t=0}^{T-1} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t) p(S_0) \nn
\eea
While this is an exact relation note that it has extra elements in the path integral where for a given reward we integrate over future MDP. This does not effect the exact results but does effect Monte Carlo methods' variance.


\subsection{Importance Sampling}

If we have two different policies $\pi$ and $b$ 
\bea
E_\pi[R_{\tau+1}]  = \sum_{S_{0:t}, A_{0:t}}  R_{\tau+1} p(R_{\tau+1}|S_\tau,A_\tau) \prod_{t=0}^{\tau-1} p(S_{t+1}|S_t,A_t) \pi(A_t|S_t)  p(S_0) \nn
E_b[R_{\tau+1}]  = \sum_{S_{0:t}, A_{0:t}}  R_{\tau+1} p(R_{\tau+1}|S_\tau,A_\tau) \prod_{t=0}^{\tau-1} p(S_{t+1}|S_t,A_t) b(A_t|S_t)  p(S_0) \nn
\eea
Then we get 
\bea
E_\pi[R_{\tau+1}] =\mathbb E_{b}[ \left( \prod_{t=0}^{\tau} \frac{\pi(A_{n} | S_{n} )}{b(A_{n} | S_{n} )} \right)   R_{\tau+1}] 
\eea
...
&=& \sum_a \partial_\theta \pi(a|s) q_\pi(s,a)  + \sum_{a,s'} \pi(a|s) p(s'|s,a) \partial_\theta v_\pi(s') \nn
&=& \sum_a \partial_\theta \pi(a|s) q_\pi(s,a) + \sum_{s'} \partial_\theta v_\pi(s') p(s'|s)
\eea
Thus recursively we get
\bea
...
\section{Explicit Policy Gradient}

Recall we have
\bea
\mathbb E_\pi[ R_{t+1}] &=&  \sum_{R_{t+1},S_{0:T}, A_{0:T-1}} R_{t+1}  \prod_{t=0}^{T-1} p(S_{t+1}|S_t,A_t) \nn
&=&  \sum_{R_{t+1},S_{0:T}, A_{0:T-1}} R_{t+1}  \prod_{t=0}^{t} p(S_{t+1}|S_t,A_t) 
\eea
